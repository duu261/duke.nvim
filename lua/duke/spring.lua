...
function M.package_name(group_id, artifact_id)
  return require("duke.maven").package_name(group_id, artifact_id)
end
...
M.adapter = {
  ...
  execute = function(opts, staging, callback)
    ...
    require("duke.process").run("curl", curl_args, { timeout = opts.timeout }, function(download)
      if download.code ~= 0 then
        fs.cleanup(archive)
        local detail = require("duke.process").detail(download)
      ...
      require("duke.process").run(
        ...
        function(inspect)
          if inspect.code ~= 0 then
            fs.cleanup(archive)
            local detail = require("duke.process").detail(inspect)
          ...
          require("duke.process").run(
            ...
            function(verbose)
              if verbose.code ~= 0 then
                fs.cleanup(archive)
                local detail = require("duke.process").detail(verbose)
                callback("Spring archive inspection failed: " .. detail)
                return
              end
              ...
              require("duke.process").run(
                ...
                function(extract)
                  ...
                  if extract.code ~= 0 then
                    local detail = require("duke.process").detail(extract)
